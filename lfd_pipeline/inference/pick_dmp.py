"""Pipeline di inferenza DMP per la primitiva PICK.

Identica a ``pick_gmm.py`` ma usa ``DMPGenerator`` (modello salvato da
``learning/dmp/dmp_train.py``). L'interfaccia ``TrajectoryGenerator`` e'
la stessa, quindi tutto il resto della pipeline (HOME, vision, esecuzione)
resta invariato.

Uso:
    python3 pick_dmp.py
    python3 pick_dmp.py --marker-id 46 --grasp-offset-z 0.0
    python3 pick_dmp.py --no-execute     # solo generazione + preview
"""
...
from inference.methods.dmp import DMPGenerator  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Inferenza DMP per PICK su xArm 6.")
    p.add_argument("--robot-ip", default=ROBOT_IP, help=f"IP xArm (default: {ROBOT_IP}).")
    p.add_argument("--model", type=Path,
                   default=TRAINED_MODELS_ROOT / "pick_dmp.npz",
                   help="Path al modello DMP (.npz).")
    ...
    p.add_argument("--close-after-traj", type=bool, default=True,
                   help="Ignora il comando di gripper generato dal motion encoder "
                        "e chiude completamente il gripper SOLO dopo che la "
                        "traiettoria DMP e' stata interamente eseguita.")
...
def main() -> int:
    ...
    print(f"[dmp] model      : {args.model}")
    print(f"[dmp] marker id  : {args.marker_id}  (lato {args.marker_len*1000:.0f} mm,"
          f" dict={ARUCO_DICT_NAME})")

    # 1) robot up + HOME (la HOME e' gia' la posa di scansione)
    arm = init_robot(args.robot_ip)
    try:
        print(f"[dmp] HOME (deg) : {HOME_JOINT_DEG}")
        ...
        live.start()
        try:
            input("[dmp] posiziona l'oggetto sotto la camera, "
                  "poi premi INVIO per acquisire la posa...")

            # 4) acquisizione posa marker -> grasp pose target
            print(f"[dmp] cerco marker id={args.marker_id} "
            ...
            if T_base_tag is None:
                raise SystemExit(f"[dmp] marker id={args.marker_id} non rilevato.")

            goal_xyzrpy = grasp_pose_from_tag(T_base_tag,
                                              offset_z_m=args.grasp_offset_z)
            print(f"[dmp] tag center (m): {T_base_tag[:3, 3].round(4).tolist()}")
            print(f"[dmp] grasp goal     : "
                  f"xyz={[round(v, 4) for v in goal_xyzrpy[:3]]}  "
                  f"rpy={[round(v, 4) for v in goal_xyzrpy[3:]]} (rad)")

            # 5) generazione traiettoria DMP
            ...
            print(f"[dmp] traj N={traj.n}  T={traj.t[-1]:.2f}s  "
                  f"dt={(traj.t[1]-traj.t[0]):.3f}s  "
                  f"grip={'si' if traj.grip is not None else 'no'}")
            print(f"[dmp] endpoint xyz   : {traj.xyz_m[-1].round(4).tolist()}")

            if args.save_csv is not None:
                save_trajectory_csv(args.save_csv, traj)
                print(f"[dmp] traiettoria salvata in {args.save_csv}")

            # 5b) preview 3D opzionale (PyBullet + URDF xArm6)
            preview = None
            if args.preview3d:
                from inference.vision.preview3d import RobotPreview3D
                print("[dmp] apro la preview 3D (PyBullet)...")
                print("[dmp] controlli camera: Ctrl+sx=ruota, "
            ...
            try:
                if args.no_execute:
                    print("[dmp] --no-execute attivo: salto l'esecuzione.")
                    if preview is None:
                        input("[dmp] INVIO per chiudere la preview...")
                else:
                    print("[dmp] esecuzione tra 2s ...")
                    time.sleep(2.0)
                    if args.close_after_traj:
                        print("[dmp] --close-after-traj: ignoro il gripper "
                    ...
                    if args.close_after_traj:
                        print(f"[dmp] chiudo il gripper a {GRIPPER_CLOSED_POS} "
                              "(post-traiettoria).")
