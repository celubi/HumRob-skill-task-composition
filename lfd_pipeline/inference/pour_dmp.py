"""Pipeline di inferenza DMP per la primitiva POUR.

Identica a ``pour_gmm.py`` ma usa ``DMPGenerator`` (modello salvato da
...
Uso:
    python3 pour_dmp.py
    python3 pour_dmp.py --marker-id 46
    python3 pour_dmp.py --no-execute     # solo generazione + preview
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Inferenza DMP per POUR su xArm 6.")
    p.add_argument("--robot-ip", default=ROBOT_IP, help=f"IP xArm (default: {ROBOT_IP}).")
    p.add_argument("--model", type=Path,
                   default=TRAINED_MODELS_ROOT / "pour_dmp.npz",
...
def main() -> int:
    ...
    print(f"[dmp-pour] model     : {args.model}")
    print(f"[dmp-pour] marker id : {args.marker_id}  (lato {args.marker_len*1000:.0f} mm,"
          f" dict={ARUCO_DICT_NAME})")
    print(f"[dmp-pour] offset    : dx={args.pour_offset_x:+.3f} m  "
          f"dy={args.pour_offset_y:+.3f} m  dz={args.grasp_offset_z:+.3f} m "
          "(assi del tag)")
    print(f"[dmp-pour] goal delta: dy={args.goal_offset_y:+.3f} m  "
    ...
    try:
        print(f"[dmp-pour] HOME (deg) : {HOME_JOINT_DEG}")
        ...
        live.start()
        try:
            input("[dmp-pour] inquadra il marker del target di pour, "
                  "poi premi INVIO per acquisire la posa...")

            # 4) acquisizione posa marker -> pour pose target
            print(f"[dmp-pour] cerco marker id={args.marker_id} "
            ...
            if T_base_tag is None:
                raise SystemExit(f"[dmp-pour] marker id={args.marker_id} non rilevato.")
            ...
            print(f"[dmp-pour] tag center (m): {T_base_tag[:3, 3].round(4).tolist()}")
            print(f"[dmp-pour] pour start    : "
                  f"xyz={[round(v, 4) for v in start_xyzrpy[:3]]}  "
                  f"rpy={[round(v, 4) for v in start_xyzrpy[3:]]} (rad)")
            print(f"[dmp-pour] pour goal     : "
            ...
            print(f"[dmp-pour] traj N={traj.n}  T={traj.t[-1]:.2f}s  "
                  f"dt={(traj.t[1]-traj.t[0]):.3f}s  "
                  f"grip={'si' if traj.grip is not None else 'no'}")
            print(f"[dmp-pour] endpoint xyz   : {traj.xyz_m[-1].round(4).tolist()}")

            if args.save_csv is not None:
                save_trajectory_csv(args.save_csv, traj)
                print(f"[dmp-pour] traiettoria salvata in {args.save_csv}")

            # 5b) preview 3D opzionale (PyBullet + URDF xArm6)
            preview = None
            if args.preview3d:
                from inference.vision.preview3d import RobotPreview3D
                print("[dmp-pour] apro la preview 3D (PyBullet)...")
                print("[dmp-pour] controlli camera: Ctrl+sx=ruota, "
            ...
            try:
                if args.no_execute:
                    print("[dmp-pour] --no-execute attivo: salto l'esecuzione.")
                    if preview is None:
                        input("[dmp-pour] INVIO per chiudere la preview...")
                else:
                    print("[dmp-pour] esecuzione tra 2s ...")
                    time.sleep(2.0)
                    # La primitiva pour non afferra: ignoriamo il segnale di
                    # gripper della policy e lasciamo lo stato corrente.
                    print("[dmp-pour] gripper: ignoro il segnale di traiettoria, "
