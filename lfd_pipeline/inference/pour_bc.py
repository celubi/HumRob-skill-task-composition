"""Pipeline di inferenza Goal-Conditioned BC per la primitiva POUR.

Stessa interfaccia di ``pour_dmp.py`` / ``pour_gmm.py``, ma usa
...
Geometria gripper rispetto al tag (frame BASE):
    - z_g = -z_t              (assi z opposti)
    - x_g = +x_t              (assi x allineati)
    - y_g = -y_t              (= z_g x x_g, destrorso e ortonormale)
La posizione del gripper e' il centro del marker spostato di
``--pour-offset-x`` lungo x_t e ``--pour-offset-y`` lungo y_t (default
+0.20 m e +0.15 m), piu' l'eventuale ``--grasp-offset-z`` lungo z_t.

Uso:
    python3 pour_bc.py
    python3 pour_bc.py --marker-id 46
    python3 pour_bc.py --no-execute        # solo generazione + preview
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Inferenza BC per POUR su xArm 6.")
    p.add_argument("--robot-ip", default=ROBOT_IP, help=f"IP xArm (default: {ROBOT_IP}).")
    p.add_argument("--model", type=Path,
                   default=TRAINED_MODELS_ROOT / "pour_bc.pt",
...
def main() -> int:
    ...
    print(f"[bc-pour] model     : {args.model}")
    print(f"[bc-pour] marker id : {args.marker_id}  (lato {args.marker_len*1000:.0f} mm,"
          f" dict={ARUCO_DICT_NAME})")
    print(f"[bc-pour] offset    : dx={args.pour_offset_x:+.3f} m  "
          f"dy={args.pour_offset_y:+.3f} m  dz={args.pour_offset_z:+.3f} m "
          "(assi del tag)")
    print(f"[bc-pour] goal delta: dy={args.goal_offset_y:+.3f} m  "
    ...
    try:
        print(f"[bc-pour] HOME (deg) : {HOME_JOINT_DEG}")
        go_home(arm, HOME_JOINT_DEG)

        # 2) gripper aperto come stato di partenza noto
        open_gripper(arm, GRIPPER_CLOSED_POS)  # stato noto, ma chiuso per sicurezza (la policy ignora il segnale di grip)
        ...
        live.start()
        try:
            input("[bc-pour] inquadra il marker del target di pour, "
                  "poi premi INVIO per acquisire la posa...")

            # 4) acquisizione posa marker -> pour pose target
            print(f"[bc-pour] cerco marker id={args.marker_id} "
            ...
            if T_base_tag is None:
                raise SystemExit(f"[bc-pour] marker id={args.marker_id} non rilevato.")
            ...
            print(f"[bc-pour] tag center (m): {T_base_tag[:3, 3].round(4).tolist()}")
            print(f"[bc-pour] pour start    : "
                  f"xyz={[round(v, 4) for v in start_xyzrpy[:3]]}  "
                  f"rpy={[round(v, 4) for v in start_xyzrpy[3:]]} (rad)")
            print(f"[bc-pour] pour goal     : "
            ...
            print(f"[bc-pour] traj N={traj.n}  T={traj.t[-1]:.2f}s  "
                  f"dt={(traj.t[1]-traj.t[0]):.3f}s  "
                  f"grip={'si' if traj.grip is not None else 'no'}")
            print(f"[bc-pour] endpoint xyz   : {traj.xyz_m[-1].round(4).tolist()}")

            if args.save_csv is not None:
                save_trajectory_csv(args.save_csv, traj)
                print(f"[bc-pour] traiettoria salvata in {args.save_csv}")

            # 5b) preview 3D opzionale (PyBullet + URDF xArm6)
            preview = None
            if args.preview3d:
                from inference.vision.preview3d import RobotPreview3D
                print("[bc-pour] apro la preview 3D (PyBullet)...")
                print("[bc-pour] controlli camera: Ctrl+sx=ruota, "
            ...
            try:
                if args.no_execute:
                    print("[bc-pour] --no-execute attivo: salto l'esecuzione.")
                    if preview is None:
                        input("[bc-pour] INVIO per chiudere la preview...")
                else:
                    print("[bc-pour] esecuzione tra 2s ...")
                    time.sleep(2.0)
                    # La primitiva pour non afferra: ignoriamo il segnale di
                    # gripper della policy e lasciamo lo stato corrente.
                    print("[bc-pour] gripper: ignoro il segnale di traiettoria, "
