"""Pipeline di inferenza GMM-GMR per la primitiva PICK.
...
Flusso:
    ...
    4. attesa rilevamento del marker dell'oggetto e calcolo della posa
       di grasping target (z gripper opposto a z tag, x allineati);
    5. generazione della traiettoria con il modello GMM-GMR addestrato
       (apertura -> avvicinamento -> chiusura -> sollevamento sono GIA'
       codificati nella traiettoria, gripper compreso);
    6. esecuzione cartesiana della traiettoria sul robot.

Uso:
    python3 pick_gmm.py
    python3 pick_gmm.py --marker-id 46 --grasp-offset-z 0.0
    python3 pick_gmm.py --no-execute     # solo generazione + preview
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Inferenza GMM-GMR per PICK su xArm 6.")
    p.add_argument("--robot-ip", default=ROBOT_IP, help=f"IP xArm (default: {ROBOT_IP}).")
    p.add_argument("--model", type=Path,
                   default=TRAINED_MODELS_ROOT / "pick_gmm.npz",
...
def main() -> int:
    ...
    print(f"[gmm] model      : {args.model}")
    print(f"[gmm] marker id  : {args.marker_id}  (lato {args.marker_len*1000:.0f} mm,"
          f" dict={ARUCO_DICT_NAME})")

    # 1) robot up + HOME (la HOME e' gia' la posa di scansione)
    arm = init_robot(args.robot_ip)
    try:
        print(f"[gmm] HOME (deg) : {HOME_JOINT_DEG}")
        ...
        live.start()
        try:
            input("[gmm] posiziona l'oggetto sotto la camera, "
                  "poi premi INVIO per acquisire la posa...")

            # 4) acquisizione posa marker -> grasp pose target
            print(f"[gmm] cerco marker id={args.marker_id} "
            ...
            if T_base_tag is None:
                raise SystemExit(f"[gmm] marker id={args.marker_id} non rilevato.")

            goal_xyzrpy = grasp_pose_from_tag(T_base_tag,
                                              offset_z_m=args.grasp_offset_z)
            print(f"[gmm] tag center (m): {T_base_tag[:3, 3].round(4).tolist()}")
            print(f"[gmm] grasp goal     : "
            ...
            print(f"[gmm] traj N={traj.n}  T={traj.t[-1]:.2f}s  "
                  f"dt={(traj.t[1]-traj.t[0]):.3f}s  "
                  f"grip={'si' if traj.grip is not None else 'no'}")
            print(f"[gmm] endpoint xyz   : {traj.xyz_m[-1].round(4).tolist()}")

            if args.save_csv is not None:
                save_trajectory_csv(args.save_csv, traj)
                print(f"[gmm] traiettoria salvata in {args.save_csv}")

            # 5b) preview 3D opzionale (PyBullet + URDF xArm6)
            preview = None
            if args.preview3d:
                from inference.vision.preview3d import RobotPreview3D
                print("[gmm] apro la preview 3D (PyBullet)...")
                print("[gmm] controlli camera: Ctrl+sx=ruota, "
            ...
            try:
                if args.no_execute:
                    print("[gmm] --no-execute attivo: salto l'esecuzione.")
                    if preview is None:
                        input("[gmm] INVIO per chiudere la preview...")
                else:
                    print("[gmm] esecuzione tra 2s ...")
                    time.sleep(2.0)
                    if args.close_after_traj:
                        print("[gmm] --close-after-traj: ignoro il gripper "
                    ...
                    if args.close_after_traj:
                        print(f"[gmm] chiudo il gripper a {GRIPPER_CLOSED_POS} "
                              "(post-traiettoria).")
