"""Pipeline di inferenza DMP per la primitiva PLACE.

Identica struttura di ``place_gmm.py`` ma usa ``DMPGenerator``.
La posa di partenza e' quella CORRENTE del TCP (l'utente posiziona il
braccio manualmente prima della scansione del marker), il goal e' la
posa di place ricavata dall'ArUco. A fine traiettoria si APRE il
gripper per rilasciare l'oggetto.

Uso:
    python3 place_dmp.py
    python3 place_dmp.py --marker-id 8 --grasp-offset-z 0.0
    python3 place_dmp.py --no-execute     # solo generazione + preview
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Inferenza DMP per PLACE su xArm 6.")
    p.add_argument("--robot-ip", default=ROBOT_IP, help=f"IP xArm (default: {ROBOT_IP}).")
    p.add_argument("--model", type=Path,
                   default=TRAINED_MODELS_ROOT / "place_dmp.npz",
...
def main() -> int:
    ...
    print(f"[dmp-place] model     : {args.model}")
    print(f"[dmp-place] marker id : {args.marker_id}  (lato {args.marker_len*1000:.0f} mm,"
